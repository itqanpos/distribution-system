/* =============================================
   pos.js - نقطة البيع (إصدار نهائي كامل مع خصم رصيد العميل)
   ============================================= */
'use strict';
...
const POS = {
    state: {
        ...
        discountValue: 0,
        netTotal: 0,
        usedCustomerBalance: 0          // الرصيد المستخدم من العميل لتغطية الفاتورة
    },
    cache: { productMap: new Map(), customerMap: new Map(), productUnitsCache: new Map() },
    el: {},
    ...
    cacheDOM() {
        const ids = [
            ...
            'paySubtotal','payDiscount','payNet','currentBalance','currentBalanceLabel',
            'receiptModal','receiptPrintArea','printReceiptBtn','cancelReceiptModalBtn','closeReceiptModalBtn',
            'moreMenuBtn','moreDropdown','holdInvoiceBtn','heldInvoicesBtn',
            'sidebarAvatar','sidebarUserName',
            'useCustomerBalanceCheckbox','customerBalanceAmount'
        ];
    ...
    bindEvents() {
        ...
        // عند تغيير اختيار استخدام الرصيد
        this.el.useCustomerBalanceCheckbox?.addEventListener('change', () => this.updatePaymentPreview());
    ...
    onCustomerSearch() {
        ...
        if (option && option.dataset.id) {
            ...
            if (customer) {
                ...
                if (bal > 0) {
                    balanceDiv.innerHTML = `العميل دائن بـ ${Utils.formatMoney(bal)} (يمكن استخدامه للدفع)`;
                    balanceDiv.className = 'customer-balance positive';
                } else if (bal < 0) {
                    balanceDiv.innerHTML = `العميل مدين بـ ${Utils.formatMoney(-bal)}`;
                    balanceDiv.className = 'customer-balance negative';
                } else {
                    balanceDiv.innerHTML = 'لا يوجد رصيد';
                    balanceDiv.className = 'customer-balance';
                }
                return;
            }
    ...
    addToCartFromModal() {
        ...
        if (price < 0) { alert('السعر لا يمكن أن يكون سالبًا'); return; }

        const unit = this.state.selectedUnit;
        if (unit) {
            if (unit.minPrice > 0 && price < unit.minPrice) {
                alert(`لا يمكن البيع بأقل من السعر الأدنى: ${Utils.formatMoney(unit.minPrice)}`);
                return;
            }
            if (unit.maxPrice > 0 && price > unit.maxPrice) {
                alert(`لا يمكن البيع بأعلى من السعر الأقصى: ${Utils.formatMoney(unit.maxPrice)}`);
                return;
            }
        }
...
openPaymentModal() {
    ...
    this.showModal('paymentModal');
        ...
        if (customer && bal > 0) {
            ...
            this.el.customerBalanceAmount.textContent = `الرصيد المتاح: ${Utils.formatMoney(bal)}`;
            // تفعيل الخيار افتراضياً
            this.el.useCustomerBalanceCheckbox.checked = true;
        } else {
            this.el.useCustomerBalanceCheckbox.style.display = 'none';
            this.el.useCustomerBalanceCheckbox.checked = false;
        }
    ...
    updatePaymentPreview() {
        ...
        const customer = this.getSelectedCustomer();
        // الرصيد المستخدم (إن وجد)
        let usedBalance = 0;
        if (customer && this.el.useCustomerBalanceCheckbox?.checked && customer.balance > 0) {
            usedBalance = Math.min(customer.balance, net); // لا يتجاوز قيمة الفاتورة
        }
        ...
        const newBal = Utils.round(currentBal - usedBalance + diff, 2); // خصم الرصيد المستخدم وإضافة الفائض أو النقص
    ...
    async completePayment() {
        ...
        try {
            ...
            // إجبارية إدخال المبلغ إذا لم يكن هناك رصيد كافٍ
            ...
            // حفظ الفاتورة وتحديث المخزون والرصيد
            const customer = this.getSelectedCustomer();
            ...
            if (this.state.isDBReady) {
                ...
                if (customer) {
                    // الرصيد الجديد = القديم - الرصيد المستخدم + الفائض (أو - المتبقي)
                ...
                // إذا تم استخدام الرصيد، فلا حاجة لحركة مالية، لإنها تسوية داخلية
            } else if (Utils.hasLocalDB()) {
    ...
    async resumeInvoice(id) {
        ...
        try {
            if (this.state.isDBReady) {
                ...
                if (inv && window.supabase) await supabase.from('invoices').delete().eq('id', id);
            } else if (Utils.hasLocalDB()) {
            ...
            if (inv.customer_id) {
                const customer = this.cache.customerMap.get(String(inv.customer_id));
                if (customer) this.el.customerSearchInput.value = customer.name || '';
    ...
    restoreCartFromStorage() {
        ...
        try {
            ...
            if (this.state.selectedCustomerId) {
                ...
                if (customer && this.el.customerSearchInput) this.el.customerSearchInput.value = customer.name || '';
