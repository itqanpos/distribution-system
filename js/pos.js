/* =============================================
   pos.js - نقطة البيع (إصدار فائق السرعة - مكتمل)
   ============================================= */
'use strict';
...
const POS = {
    ...
    async loadInitialData() {
        ...
        if (!this.state.products.length) this.showToast('لا توجد منتجات');
    },
    ...
    openUnitModal(productId) {
        ...
        if (!product || !product.units?.length) { this.showToast('المنتج غير موجود'); return; }
    ...
    addToCartFromModal() {
        ...
        if (qty <= 0 || qty > (+this.el.selectedQuantity?.max || 0)) { alert('كمية غير متاحة'); return; }
    ...
    async completePayment() {
        ...
        } catch (error) { console.error('خطأ في الدفع:', error); alert('حدث خطأ أثناء الدفع'); }
        finally { this.state.isProcessing = false; this.el.confirmAndPrintBtn.disabled = false; }
    },
    ...
    async resumeInvoice(id) {
        ...
        try {
            if (this.state.isDBReady) {
                ...
                inv = (await localDB.getAll('invoices')).find(i => i.id === id);
        ...
        } catch (err) { console.error(err); alert('فشل استرجاع الفاتورة'); }
    },
    ...
    showReceiptModal(invoice, customer, items, totals, oldBalance = 0) {
        ...
        const itemsRows = items.map(item => `<tr><td>${Utils.escapeHTML(item.productName)} - ${Utils.escapeHTML(item.unitName)}</td><td>${item.quantity}</td><td>${Utils.formatMoney(item.price)}</td><td>${Utils.formatMoney(Utils.round(item.price * item.quantity, 2))}</td></tr>`).join('');
        ...
        this.el.receiptPrintArea.innerHTML = `
            ...
            <p><strong>العميل:</strong> ${Utils.escapeHTML(customer?.name || 'نقدي')}</p>
            <p><strong>رقم الفاتورة:</strong> ${invoice.invoice_number || invoice.id?.substring(0,8)}</p>
            <p><strong>التاريخ:</strong> ${Utils.formatDate(invoice.date)}</p>
            <div class="divider"></div>
