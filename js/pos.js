/* =============================================
   pos.js - نقطة البيع (إصدار 8.5 - نهائي ومستقر)
   - بدون باركود نهائياً
   - جميع الأزرار مربوطة بشكل صحيح
   - معالجة أخطاء شاملة
   - تحميل بيانات قسري من السحابة
   - دعم كامل للدفع الآجل
   ============================================= */
'use strict';
...
const POS = {
    ...
    async init() {
        ...
        try {
            ...
            console.error('❌ فشل تهيئة POS:', e);
            if (window.Toast) Toast.error('حدث خطأ أثناء تهيئة نقطة البيع');
        }
    },
    ...
    _bind() {
        const on = (id, ev, fn) => {
            if (this.el[id]) {
                this.el[id].addEventListener(ev, fn);
            } else {
        ...
        on('moreMenuBtn', 'click', (e) => {
            e.stopPropagation();
            console.log('moreMenuBtn clicked');
        ...
        on('confirmAndPrintBtn', 'click', (e) => {
            e.preventDefault();
            console.log('confirmAndPrintBtn clicked');
    ...
    _filterCustomers() {
        ...
        const dd = this.el.customerDropdown;
        if (!dd) return;
    ...
    _updateCustDisplay() {
        const el = this.el.customerBalanceDisplay;
        if (!el) return;
    ...
    _calcTotals() {
        let sub = 0;
        for (const i of this.state.cart) sub += U.round(i.price * i.quantity);
        sub = U.round(sub, 2);
    ...
    _updateTotals() {
        ...
        let pcs = 0;
        for (const i of this.state.cart) pcs += i.quantity * (i.factor || 1);
    ...
    _renderCart() {
        const c = this.el.cartItemsContainer;
        if (!c) return;
    ...
    _openUnitModal(id) {
        ...
        this.state.selectedUnit = p.units[0];
        ...
        this._showModal('unitQuantityModal');
    },
    ...
    _updateUnitInfo() {
        const p = this.state.selectedProduct, u = this.state.selectedUnit;
        if (!p || !u) return;
        ...
        this.el.selectedQuantity.max = max;
        this.el.selectedQuantity.value = max > 0 ? 1 : 0;
    ...
    async _completePayment() {
        ...
        try {
            ...
            if (this.state.resumedInvoiceId) {
                try {
                    await window.supabaseClient.from('invoices').delete().eq('id', this.state.resumedInvoiceId);
                } catch {}
            ...
            this._updateStock();
            if (this.state.db && navigator.onLine) {
                await this._fetchProdsAndCusts();
                this._buildCache();
            }
            this._renderProductGrid();
    ...
    _getCust() {
        return this.state.selectedCustomerId ? this.cache.custs.get(this.state.selectedCustomerId) : null;
    },
    ...
    async holdInvoice() {
        ...
        const inv = {
            id: U.uuid(),
            invoice_number: this.state.db ? await DB.generateInvoiceNumber() : this._localInvNum(),
            type: 'sale',
            date: U.today(),
            customer_id: this.state.selectedCustomerId || null,
    ...
    async loadHeld() {
        ...
        const c = this.el.heldInvoicesList;
        if (!c) return;
        c.innerHTML = invs.length ? invs.map(i => `
            <div class="held-invoice-item" data-id="${i.id}" style="padding:15px;border:1px solid ${U.cssVar('--border-light', '#e2e8f0')};border-radius:12px;margin-bottom:10px;cursor:pointer;display:flex;justify-content:space-between;">
                <div><strong>${U.escape(i.invoice_number || i.id?.substring(0, 8))}</strong><br>${U.escape(i.customer_name || 'نقدي')} - ${U.fmtMoney(i.total)}</div>
                <div><i class="fas fa-play"></i></div>
            </div>
        `).join('') : '<p style="text-align:center;">لا توجد فواتير معلقة</p>';
    ...
    _showReceipt(inv, cust, items, totals, oldBal, pay) {
        ...
        const receiptHtml = `
            <div style="font-family:'Cairo',sans-serif;font-size:13px;line-height:1.5;text-align:right;direction:rtl;padding:10px;width:80mm;max-width:100%;margin:0 auto;background:white;">
                <div style="text-align:center;font-weight:bold;font-size:16px;margin-bottom:5px;">${U.escape(name)}</div>
                ${phone ? `<div style="text-align:center;font-size:12px;margin-bottom:10px;">هاتف: ${U.escape(phone)}</div>` : ''}
                <hr style="border-top:1px dashed #000;margin:10px 0;">
                <div style="display:flex;justify-content:space-between;"><span>العميل:</span> <strong>${U.escape(cust?.name || 'نقدى')}</strong></div>
                <div style="display:flex;justify-content:space-between;"><span>رقم الفاتورة:</span> <strong>${U.escape(inv.invoice_number || inv.id?.substring(0, 8))}</strong></div>
                <div style="display:flex;justify-content:space-between;"><span>التاريخ:</span> ${U.fmtDate(inv.date)}</div>
                <hr style="border-top:1px dashed #000;margin:10px 0;">
                <table style="width:100%;border-collapse:collapse;font-size:12px;">
                    <thead><tr><th style="text-align:right;">الصنف</th><th style="text-align:center;">كمية</th><th style="text-align:center;">سعر</th><th style="text-align:left;">إجمالي</th></tr></thead>
                ...
                <div style="display:flex;justify-content:space-between;font-weight:bold;font-size:14px;"><span>الصافي:</span> ${fmt(totals.net)}</div>
                <hr style="border-top:1px dashed #000;margin:10px 0;">
                ...
                ${used > 0 ? `<div style="display:flex;justify-content:space-between;"><span>من رصيد:</span> ${fmt(used)}</div>` : ''}
                ...
                ${diff > 0 ? `<div style="display:flex;justify-content:space-between;color:green;"><span>فائض (أضيف للرصيد):</span> ${fmt(diff)}</div>` : ''}
                ${diff < 0 ? `<div style="display:flex;justify-content:space-between;color:red;"><span>متبقي:</span> ${fmt(-diff)}</div>` : ''}
                ${cust && cust.name !== 'نقدي' ? `
                    <hr style="border-top:1px dashed #000;margin:10px 0;">
                    <div style="display:flex;justify-content:space-between;"><span>الرصيد السابق:</span> ${fmt(oldBal)}</div>
                    ${used > 0 ? `<div style="display:flex;justify-content:space-between;"><span>خصم:</span> -${fmt(used)}</div>` : ''}
                    ${diff > 0 ? `<div style="display:flex;justify-content:space-between;color:green;"><span>إضافة:</span> +${fmt(diff)}</div>` : ''}
                    ${diff < 0 ? `<div style="display:flex;justify-content:space-between;color:red;"><span>متبقي للدفع:</span> ${fmt(-diff)}</div>` : ''}
                    <div style="display:flex;justify-content:space-between;font-weight:bold;"><span>الرصيد الحالي:</span> ${fmt(newBalance)}</div>
                ` : ''}
                <hr style="border-top:1px dashed #000;margin:10px 0;">
                <div style="text-align:center;font-weight:bold;">${U.escape(foot)}</div>
            </div>
        `;
    ...
    _restoreCart() {
        ...
        try {
            ...
            if (d.ts && Date.now() - d.ts > 2 * 60 * 60 * 1000) {
                localStorage.removeItem('pos_cart');
                return;
            }
            this.state.cart = d.cart || [];
            ...
            this._renderCart();
            if (d.cust) {
                const c = this.cache.custs.get(String(d.cust));
                if (c) this.el.customerSearchInput.value = c.name || '';
                this._updateCustDisplay();
            }
        } catch {
