/* =============================================
   pos.js - نقطة البيع (نسخة نهائية مصححة)
   - بدون كاميرا
...
const POS = {
    ...
    _calcTotals() {
        ...
        sub = U.round(sub, 2);
        let disc = this.state.discountType === 'amount'
            ? Math.min(this.state.discountValue, sub)
            : U.round(sub * this.state.discountValue / 100, 2);
    ...
    async _confirmAction(message) {
        if (window.ModalConfirm && ModalConfirm.show) {
            return await ModalConfirm.show({ title: 'تأكيد', message, icon: 'warn', confirmText: 'نعم', cancelText: 'إلغاء' });
        }
    ...
    _getCust() {
        return this.state.selectedCustomerId
            ? this.cache.custs.get(this.state.selectedCustomerId)
            : null;
    },
    ...
    async _printThermal() {
        try {
            ...
            encoder.align('ct').size(1, 1).text('حسابي').text('---------------------').align('rt').text(receipt).cut('partial');
...
// استدعاء تلقائي بعد التأكد من جاهزية النواة
(async function autoInit() {
    ...
    try {
        ...
        console.error('POS auto init failed:', e);
        const loadingBar = document.getElementById('loading-bar');
        if (loadingBar) loadingBar.style.background = 'var(--danger)';
        U.safeToast('فشل تحميل نقطة البيع', 'error');
    }
})();
