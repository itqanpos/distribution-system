...
(function() {
    ...
    POS._bindEvents = function() {
        ...
        on('logoutBtn', 'click', async (e) => { e.preventDefault(); const confirmed = await POS._confirmAction('هل أنت متأكد من تسجيل الخروج؟'); if (confirmed) App.logout(); });

        on('quickSaleToggle', 'click', () => { POS.state.quickSale = !POS.state.quickSale; UserPrefs.set('quickSale', POS.state.quickSale); POS.el.quickSaleToggle?.classList.toggle('active', POS.state.quickSale); U.showToast(POS.state.quickSale ? 'البيع السريع مفعّل' : 'البيع السريع معطّل'); });
        ...
        on('productDropdown', 'click', (e) => {
            ...
            if (item?.dataset.id) { POS._openUnitModal(item.dataset.id); POS._hideProdDropdown(); POS.el.productSearchInput.value = ''; }
        });
        ...
        on('customerDropdown', 'click', (e) => {
            ...
            if (item?.dataset.id) {
                ...
                POS._updateCustDisplay(); POS._hideCustDropdown(); POS._saveCart();
            }
        });
    ...
    POS._setupBarcodeBuffer = function() {
        document.addEventListener('keydown', (e) => {
            ...
            const now = Date.now();
            if (e.key === 'Enter') {
                if (POS.state._barcodeBuffer.length > 5) POS._searchBarcode(POS.state._barcodeBuffer);
                POS.state._barcodeBuffer = ''; POS.state._lastKeyTime = 0;
                return;
            }
            if (e.key.length === 1) {
                if (POS.state._lastKeyTime && (now - POS.state._lastKeyTime > 30)) POS.state._barcodeBuffer = '';
    ...
    POS._onCartChange = function(e) {
        if (e.target.classList.contains('cart-price-input')) {
            if (!POS._canChangePrice()) { U.showToast('ليس لديك صلاحية لتغيير السعر', 'error'); POS._renderCart(); return; }
