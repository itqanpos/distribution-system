...
(function() {
    ...
    POS._resetCart = function() {
        ...
        POS.state.taxRate = 0; POS.state.taxAmount = 0; POS.state.shipping = 0;
        if (POS.el.discountValue) POS.el.discountValue.value = 0;
        if (POS.el.discountType) POS.el.discountType.value = 'amount';
        if (POS.el.taxRate) POS.el.taxRate.value = 0;
        if (POS.el.shipping) POS.el.shipping.value = 0;
    ...
    POS._calcTotals = function() {
        let sub = 0; for (const i of POS.state.cart) sub += U.round(i.price * i.quantity, 2);
        sub = U.round(sub, 2);
        let disc = POS.state.discountType === 'amount' ? Math.min(POS.state.discountValue, sub) : U.round(sub * POS.state.discountValue / 100, 2);
        const afterDiscount = U.round(sub - disc, 2);
        const tax = U.round(afterDiscount * POS.state.taxRate / 100, 2);
        const net = U.round(afterDiscount + tax + POS.state.shipping, 2);
        POS.state.subtotal = sub; POS.state.discount = disc; POS.state.taxAmount = tax; POS.state.net = net;
        return { sub, disc, tax, net, shipping: POS.state.shipping };
    };
    POS._updateTotals = function() {
        ...
        if (POS.el.discount) POS.el.discount.textContent = disc > 0 ? `-${U.fmtMoney(disc)}` : '0.00 ج.م';
        if (POS.el.taxAmount) POS.el.taxAmount.textContent = U.fmtMoney(tax);
        if (POS.el.shippingAmount) POS.el.shippingAmount.textContent = U.fmtMoney(shipping);
        ...
        if (POS.el.profitDisplay) {
            if (POS.state.currentUser?.role === 'admin' && POS.state.cart.length > 0) {
                ...
                POS.el.profitDisplay.textContent = `الربح المتوقع: ${U.fmtMoney(sub - disc - totalCost)}`;
            } else POS.el.profitDisplay.style.display = 'none';
        }
    };
    POS._saveCart = function() {
        const state = { cart: POS.state.cart, cust: POS.state.selectedCustomerId, discType: POS.state.discountType, discVal: POS.state.discountValue, taxRate: POS.state.taxRate, shipping: POS.state.shipping, editingInv: POS.state.editingInv, usedBalance: POS.state.usedBalance, resumedInvoiceId: POS.state.resumedInvoiceId, ts: Date.now() };
    ...
    POS._restoreCart = function() {
        ...
        try {
            ...
            POS.state.taxRate = d.taxRate || 0; POS.state.shipping = d.shipping || 0;
            ...
            if (POS.el.taxRate) POS.el.taxRate.value = POS.state.taxRate;
            if (POS.el.shipping) POS.el.shipping.value = POS.state.shipping;
            POS._renderCart();
    ...
    POS._savePaymentDraft = function() {
        ...
        const draft = { cart: POS.state.cart, customerId: POS.state.selectedCustomerId, discountValue: POS.state.discountValue, discountType: POS.state.discountType, taxRate: POS.state.taxRate, shipping: POS.state.shipping, usedBalance: POS.state.usedBalance, editingInv: POS.state.editingInv, resumedInvoiceId: POS.state.resumedInvoiceId, paymentMethod: POS.el.paymentMethod?.value, paymentNotes: POS.el.paymentNotes?.value, cashAmount: POS.el.cashAmount?.value, transferAmount: POS.el.transferAmount?.value, modalOpen: POS.el.paymentModal?.classList.contains('open') };
    ...
    POS.getActivityLog = function() {
        try { return JSON.parse(localStorage.getItem('pos_activity_log') || '[]'); } catch { return []; }
    };
})();
