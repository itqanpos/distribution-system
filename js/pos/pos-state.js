'use strict';
import { UserPrefs } from './utils.js';
...
// دوال تعديل الحالة الأساسية
export function _resetCart(POS) {
    POS.state.cart = [];
...
export function _getCust(POS) {
    return POS.state.selectedCustomerId ? POS.cache.custs.get(POS.state.selectedCustomerId) : null;
}

export function _calcTotals(POS) {
    let sub = 0;
    for (const i of POS.state.cart) sub += POS.U.round(i.price * i.quantity);
    sub = POS.U.round(sub, 2);
    let disc = POS.state.discountType === 'amount'
        ? Math.min(POS.state.discountValue, sub)
        : POS.U.round(sub * POS.state.discountValue / 100, 2);
    const net = POS.U.round(sub - disc, 2);
...
export function _updateTotals(POS) {
    const { sub, net } = POS._calcTotals();
    if (POS.el.subtotal) POS.el.subtotal.textContent = POS.U.fmtMoney(sub);
    if (POS.el.netTotal) POS.el.netTotal.textContent = POS.U.fmtMoney(net);
    if (POS.el.itemTypesCount) POS.el.itemTypesCount.textContent = POS.state.cart.length;
    let pcs = 0;
    for (const i of POS.state.cart) pcs += i.quantity * (i.factor || 1);
    ...
    if (POS.el.profitDisplay) {
        if (POS.state.currentUser?.role === 'admin' && POS.state.cart.length > 0) {
            let totalCost = 0;
            for (const i of POS.state.cart) totalCost += (i.cost || 0) * i.quantity;
            const profit = sub - totalCost;
            POS.el.profitDisplay.style.display = 'block';
            POS.el.profitDisplay.textContent = `Expected profit: ${POS.U.fmtMoney(profit)}`;
        } else {
...
export function _restoreCart(POS) {
...
export function _savePaymentDraft(POS) {
...
export function _restorePaymentDraft(POS) {
...
export function _logActivity(POS, action, details) {
    POS.state._activityLog.push({
        action,
        details,
        user: POS.state.currentUser?.fullName,
        time: new Date().toISOString()
    });
}

export function _localInvNum() {
