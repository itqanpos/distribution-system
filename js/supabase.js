/* =============================================
   supabase.js - النواة المُوحَّدة (v3.1)
   مع وضع الطوارئ للعمل دون اتصال أو فشل CDN
   ============================================= */
(function() {
    'use strict';

    // تعريف كائنات أساسية فارغة حتى لو فشل كل شيء
    ...
    function tryInitSupabase() {
        ...
        try {
            supabaseClient = supabase.createClient(SUPABASE_URL, SUPABASE_ANON_KEY, {
                ...
                global: {
                    headers: { 'X-Client-Info': 'supplier-portal/3.1' }
                }
            });
    ...
    // إذا نجحت التهيئة، نعرّف كل الوظائف الحقيقية
    function setupFullApp() {
        ...
        const OfflineLayer = {
            ...
            async save(storeName, data, cloudSaver) {
                ...
                if (local) await local.put(storeName, data).catch(() => {});
        ...
        window.App = {
            ...
            async checkTenantStatus(tenantId) {
                if (!supabaseClient) return;
                try {
                    const { data: tenant } = await supabaseClient.from('tenants').select('plan').eq('id', tenantId).single();
                    if (tenant?.plan === 'expired') window.location.href = './expired.html';
                } catch (e) {}
            },
        ...
        window.DB = {
            async getProducts(forceRefresh) {
                return OfflineLayer.get('products', async () => {
                    const { data, error } = await supabaseClient.from('products').select('*, product_units(*)').order('name');
            ...
            async getSettings() {
                ...
                if (!tenantId) return {};
                try {
                    const { data, error } = await supabaseClient.from('settings').select('data').eq('tenant_id', tenantId).single();
            ...
            async saveSettings(s) {
                ...
                const { data, error } = await supabaseClient.from('settings')
                    .upsert({ tenant_id: tenantId, data: s }, { onConflict: 'tenant_id' }).select().single();
                if (error) throw error;
                SessionStore._settings = data.data;
                return data.data;
            },
            ...
            async generateInvoiceNumber() {
                ...
                const { data, error } = await supabaseClient.rpc('next_sequence', { p_tenant_id: tenantId, p_name: seqName });
                if (error) {
                    const { data: seqData } = await supabaseClient.from('sequences').select('value')
                        .eq('tenant_id', tenantId).eq('name', seqName).single();
                    let newValue = 1;
                    if (seqData) {
            ...
            async _localLightFallback(store) {
                ...
                if (local) {
                    ...
                    return all.map(item => Object.fromEntries(fields.map(f => [f, item[f]])))
        ...
        console.log('✅ نظام متعدد المستأجرين جاهز (v3.1)');
    }

    // وضع الطوارئ - إذا فشلت التهيئة، نعرّف كائن App وهمي برسائل خطأ مفيدة
    function setupFallbackApp() {
        window.App = {
            getCurrentUser: async () => null,
            getTenantId: async () => null,
            login: async () => { throw new Error('الخادم غير متاح حالياً'); },
            signup: async () => { throw new Error('الخادم غير متاح حالياً'); },
    ...
    if (tryInitSupabase()) {
        ...
        setupFallbackApp();
        // إعادة المحاولة عند تحميل المكتبة لاحقًا
