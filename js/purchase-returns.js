...
const PurchaseReturns = {
    ...
    async loadData() {
        try {
            if (Utils.isDBReady()) {
                ...
                this.returns = []; this.purchases = []; this.products = []; this.suppliers = [];
            }
    ...
    populateInvoiceDatalist() {
        ...
        this.el.invoiceList.innerHTML = this.purchases.map(p => {
            const supplierName = p.supplier_name || 'غير معروف';
            return `<option value="${p.id?.substring(0, 8)} - ${supplierName}" data-id="${p.id}">${p.id?.substring(0, 8)} - ${supplierName}</option>`;
        }).join('');
    },
    ...
    onInvoiceSearch() {
        ...
        if (option?.dataset.id) this.loadInvoiceItems(option.dataset.id);
    },

    loadInvoiceItems(invoiceId) {
        const invoice = this.purchases.find(i => i.id === invoiceId);
        if (!invoice) return;
        ...
        if (returnType === 'full_invoice') {
            ...
            (invoice.items || []).forEach((item, idx) => {
                html += `<div class="invoice-item-row">
                    <input type="checkbox" id="item_${idx}" data-idx="${idx}" onchange="PurchaseReturns.updateReturnTotal()">
                    <span class="item-info">${item.productName} - ${item.unitName}</span>
                    <input type="number" class="item-qty" value="${item.quantity}" min="0.001" max="${item.quantity}" step="0.001" data-idx="${idx}" oninput="PurchaseReturns.updateReturnTotal()" disabled>
    ...
    updateReturnTotal() {
        ...
        if (returnType === 'full_invoice') {
            ...
            if (option?.dataset.id) {
                const invoice = this.purchases.find(i => i.id === option.dataset.id);
            ...
            if (option?.dataset.id) {
                ...
                if (invoice?.items) {
                    invoice.items.forEach((item, idx) => {
                        ...
                        const qtyInput = document.querySelector(`.item-qty[data-idx="${idx}"]`);
            ...
            if (product?.units) {
                this.el.directProductUnits.innerHTML = `
                    <div class="form-group"><label>الوحدة</label>
                        <select id="directUnitSelect" onchange="PurchaseReturns.onDirectUnitChange()">
                            ${product.units.map((u, i) => `<option value="${i}" data-price="${u.cost || 0}">${u.name}</option>`).join('')}
                        </select>
                    </div>
                `;
    ...
    onDirectUnitChange() {
        ...
        if (select) {
            ...
            if (selected?.dataset.price) this.el.directPrice.value = selected.dataset.price;
        }
        this.updateReturnTotal();
    },
    ...
    closeModal() { this.el.returnModal.classList.remove('open'); },
    ...
    async saveReturn() {
        ...
        let total = 0, items = [], invoiceId = null, supplierName = '';
        ...
        try {
            if (returnType === 'full_invoice') {
                ...
                invoice.items.forEach((item, idx) => {
                    ...
                    if (cb?.checked && qtyInput) {
                        ...
                        if (qty > 0) { items.push({ ...item, quantity: qty }); total += qty * item.price; }
                    }
                });
            ...
            const returnData = {
                ...
                date, type: 'purchase_return', invoice_id: invoiceId,
                supplier_name: supplierName, items, total, reason, method
            };

            if (Utils.isDBReady()) {
                // await DB.savePurchaseReturn(returnData);
            } else if (Utils.hasLocalDB()) {
                await localDB.put('purchase_returns', returnData);
            }
            this.closeModal();
            await this.loadData();
...
document.addEventListener('DOMContentLoaded', () => PurchaseReturns.init());
