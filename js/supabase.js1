/* =============================================
   ...
   الإصدار 3.3 - إنتاجي كامل، آمن، ومقاوم للأخطاء
   ============================================= */
(function() {
    'use strict';
    ...
    // ==================== مساعد التوافقية (Polyfills) ====================
    function generateUUID() {
        if (typeof crypto !== 'undefined' && crypto.randomUUID) {
            return crypto.randomUUID();
        }
        // بديل لبيئات HTTP المحلية (Local Network) حيث لا يعمل randomUUID
    ...
    const SessionStore = {
        ...
        set user(val) {
            ...
            if (val) {
                // حفظ البيانات الأساسية فقط لأغراض العرض (UI Routing)، والأمان يعتمد على JWT
                const session = {
                    id: val.id,
                    email: val.email,
        ...
        async _backgroundSync(storeName, cloudFetcher, local) {
            try {
                ...
                if (cloudData && cloudData.length > 0) {
                    // تحسين: التحقق من التضارب باستخدام _updated_at إذا كان متوفراً
        ...
        async _queueForSync(table, data) {
            ...
            if (local && local.addToSyncQueue) {
                await local.addToSyncQueue({
                    id: data.id, // استخدام ID لمنع التكرار
                    type: data.id ? 'UPDATE' : 'INSERT',
                    table,
                    data,
                    retries: 0 // تتبع محاولات إعادة المزامنة
        ...
        async _processItem(item, local) {
            try {
                ...
                if (handler) {
                    ...
                    // نجاح العملية: إزالة العنصر من الطابور
                    if (local.removeFromSyncQueue) await local.removeFromSyncQueue(item.id || item.data.id);
                } else {
                ...
                // إدارة الفشل وتفادي الدوران اللانهائي
                item.retries = (item.retries || 0) + 1;
                if (item.retries >= 3) {
                    console.error('تم تجاوز الحد الأقصى للمحاولات. سيتم إزالة العنصر من الطابور لتجنب تعطيل النظام:', item);
                    if (local.removeFromSyncQueue) await local.removeFromSyncQueue(item.id || item.data.id);
                } else {
    ...
    function tryInitSupabase() {
        ...
        try {
            supabaseClient = supabase.createClient(SUPABASE_URL, SUPABASE_ANON_KEY, {
                ...
                global: {
                    headers: { 'X-Client-Info': 'supplier-portal/3.3' }
                }
            });
    ...
    function setupFullApp() {
        ...
        window.App = {
            ...
            async login(email, password) {
                ...
                if (profile.role !== 'super_admin' && !profile.tenant_id) {
                    ...
                    // يتم جلب auth.uid() من داخل الـ RPC للأمان
        ...
        window.DB = {
            ...
            saveProduct: p => OfflineLayer.save('products', { ...p, id: p.id || generateUUID() }, async (product) => {
                const { data, error } = await supabaseClient
                    .from('products')
            ...
            saveParty: p => OfflineLayer.save('parties', { ...p, id: p.id || generateUUID() }, async (party) => {
                const { data, error } = await supabaseClient
                    .from('parties')
            ...
            saveRep: r => OfflineLayer.save('reps', { ...r, id: r.id || generateUUID() }, async (rep) => {
                const { data, error } = await supabaseClient
                    .from('reps')
            ...
            saveInvoice: inv => OfflineLayer.save('invoices', { ...inv, id: inv.id || generateUUID() }, async (invoice) => {
                const { data, error } = await supabaseClient
                    .from('invoices')
            ...
            savePurchase: pur => OfflineLayer.save('purchases', { ...pur, id: pur.id || generateUUID() }, async (purchase) => {
                const { data, error } = await supabaseClient
                    .from('purchases')
            ...
            saveTransaction: t => OfflineLayer.save('transactions', { ...t, id: t.id || generateUUID() }, async (trans) => {
                const { data, error } = await supabaseClient
                    .from('transactions')
            ...
            saveReturn: r => OfflineLayer.save('returns', { ...r, id: r.id || generateUUID() }, async (ret) => {
                const { data, error } = await supabaseClient
                    .from('returns')
            ...
            async getSettings() {
                ...
                try {
                    // الاعتماد على RLS في قاعدة البيانات لجلب إعدادات المستخدم المتصل فقط
                    const { data, error } = await supabaseClient
                        .from('settings')
                        .select('data')
                        .single(); // ستعمل بشكل صحيح إذا كانت RLS مُفعلة ومربوطة بـ auth.uid()
            ...
            async saveSettings(s) {
                ...
                // يتم الاعتماد على قاعدة البيانات لتحديث/إدراج بناءً على auth.uid() داخل RLS أو التريجر
                // لتجنب تمرير tenantId من الفرونت إند والتلاعب به
                const { data, error } = await supabaseClient
                    .from('settings')
                    .upsert({ data: s }) // تأكد أن السكيما في Postgres تعالج tenant_id افتراضياً
                    .select()
                    .single();
                if (error) throw error;
                SessionStore._settings = data.data;
            ...
            saveJournalEntry: e => OfflineLayer.save('journal_entries', { ...e, id: e.id || generateUUID() }, async (entry) => {
                const { data, error } = await supabaseClient
                    .from('journal_entries')
            ...
            async generateInvoiceNumber() {
                ...
                // ملاحظة أمنية: تم إزالة p_tenant_id لأن دالة next_sequence يجب أن تستخرج المستأجر من auth.uid() برمجياً داخل قاعدة البيانات.
                const { data, error } = await supabaseClient.rpc('next_sequence', {
                    p_name: seqName
                });

                if (error) {
                    console.warn("استرجاع التسلسل من الـ RPC فشل، جاري المحاولة يدوياً كبديل أخير.");
                    // بديل طوارئ في حال عدم وجود الـ RPC، ويعتمد أيضاً على RLS
        ...
        console.log('✅ نظام متعدد المستأجرين جاهز (v3.3)');
    }
